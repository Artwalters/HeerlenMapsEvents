...
console.log("Test2 script is geladen!");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  testDiv.innerHTML = 'Test2 script werkt!';
...
const CONFIG = {
  ...
  async function navigateMarkers(direction) {
    ...
    // Start the flyTo animation immediately
    const flyToPromise = map.flyTo({
      center: coordinates,
    ...
    // 1. First, animate out the current popup and sidebar item
    const animateOutPromises = [];
    
    // Handle existing popup animation
    if (activePopup) {
      ...
      // Create a promise for the popup animation
      const popupPromise = new Promise(resolve => setTimeout(resolve, 400));
      animateOutPromises.push(popupPromise);
      
      // When animation completes, remove the popup (only if no newer request has come in)
      popupPromise.then(() => {
    ...
    // Handle existing sidebar item animation
    ...
    if (visibleItem.length) {
      ...
      // Create a promise for the sidebar animation
      const sidebarPromise = new Promise(resolve => setTimeout(resolve, 400));
      animateOutPromises.push(sidebarPromise);
      
      // When animation completes, remove the is--show class (only if no newer request has come in)
      sidebarPromise.then(() => {
        if (currentRequestId === navigationRequestId) {
    ...
    // 2. Wait for both the fly animation and fade-out animations to complete
    await Promise.all([flyToPromise, ...animateOutPromises]);
    
    // Check if this request is still the latest one
    if (currentRequestId !== navigationRequestId) {
      // A newer navigation request has come in, stop this one
      return;
    }
    
    // 3. Now add the new popup
    ...
    // 4. Update sidebar with proper animation
    if (properties.type !== 'ar') {
      // Show sidebar wrapper if not already visible
      $(".locations-map_wrapper").addClass("is--show");
      
      // Prepare the new item for animation
      ...
      // For AR locations, hide the sidebar
